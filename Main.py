...
DBLOCATION = "Your_NCBI_DB.xml" #SNP DB location
...
def exportSeq(DB):
    ...
    try:
        with open('AnalysedList.csv', 'w') as csvfile:
            ...
            spamwriter.writerow(["srID","geneID","geneName","Clinical Significance","WT sequance","Variants","chromosome"]) #column titles
            rows = []
            print("Building rows")
            for seq in DB: #data
                if seq.geneName == None:
                    seq.geneName = "NA"
                stringified = str(seq.snpID+','+seq.geneID+','+seq.geneName+','+seq.clinical+','+seq.seq3[-20:] + "<" + seq.wt + ">" + seq.seq5[:20]+','+"|".join(seq.varSeqList)+','+seq.chromo)
